...
from Model import *
# removed import due to circular sadness uhhhhh
...
class Simulator(Thread):
    ...
    def run(self):
        ...
        while (not self.stopper.is_set()) and self.index < (len(self.Xks)):
            ...
            X_k_p1 = A(self.dt).dot(X_k) + self.dt / 6 * \
                (K1 + 2 * K2 + 2 * K3 + K4) + W
            if self.no_noise:
                X_k_p1 = A(self.dt).dot(X_k) + self.dt / \
                    6 * (K1 + 2 * K2 + 2 * K3 + K4)
            # print A(self.dt)
            # print X_k_p1[0:3]
...
def main():
    ...
    CC = Controller(CC_stopper, CRC, sh, Xks, Uks, r_wheel, dt, Q,
                    R, T, maxUc, LogQ, not RunControl, speedup, timelock, LogQ2)

    VSim_stopper = Event()
    # setup the simulator
    VSim = Simulator(VSim_stopper, CRC, sh, Xks,
                     r_wheel, dt, Q, speedup, timelock)
    CRC.start()
    VSim.start()
    # time.sleep(0.005)
    CC.start()
