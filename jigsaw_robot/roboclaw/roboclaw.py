...
##		self._port.write(chr(address).encode())
		self._port.write(int.to_bytes(address, 1, 'big'))
		self.crc_update(command)
##		self._port.write(chr(command).encode())
		self._port.write(int.to_bytes(command, 1, 'big'))
		return
...
##		self._port.write(chr(val&0xFF).encode())
		self._port.write(int.to_bytes(val&0xFF, 1, 'big'))
	...
	def SendRandomData(self,cnt):
		for i in range(0,cnt):
			...
			self._port.write(int.to_bytes(byte, 1, 'big'))
		return
